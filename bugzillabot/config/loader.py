...
CONFIG_FILES_PATH = ["~/bot_bugzilla.conf", "/etc/bot_bugzilla.conf", "/etc/bot_bugzilla/bot_bugzilla.conf"]
...
def load_config():
    ...
    for config_file_path in CONFIG_FILES_PATH:
        expanded_config_file_path = os.path.expanduser(config_file_path)
